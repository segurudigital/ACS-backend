...
class HierarchicalMigration {
  ...
  async run() {
    try {
      console.log('=== Hierarchical System Migration ===');
      ...
      if (!validation.isValid) {
        ...
        if (!this.dryRun) {
          throw new Error('Cannot proceed with migration due to validation errors');
        }
      }
      ...
      const postValidation = await hierarchyMigrationService.validateHierarchyIntegrity();
  ...
  async validatePreMigration() {
    ...
    if (orgsWithoutType > 0) {
      errors.push(`${orgsWithoutType} organizations missing type/hierarchyLevel`);
    }
      ...
      for (const org of orgs) {
        try {
          ...
          if (oldPath !== org.hierarchyPath) {
            ...
            if (this.verbose) {
              console.log(`  Updated ${org.name}: ${oldPath} → ${org.hierarchyPath}`);
            }
          }
        } catch (error) {
          this.stats.organizations.errors++;
    ...
    for (const team of teams) {
      try {
        ...
        if (updated) {
          ...
          if (this.verbose) {
            console.log(`  Updated ${team.name}: ${oldPath} → ${team.hierarchyPath}`);
          }
        }
      } catch (error) {
    ...
    for (const service of services) {
      try {
        ...
        if (updated) {
          ...
          if (this.verbose) {
            console.log(`  Updated ${service.name}: ${oldPath} → ${service.hierarchyPath}`);
          }
        }
      } catch (error) {
        ...
        if (user.organizations && user.organizations.length > 0) {
          ...
          for (const orgAssignment of user.organizations) {
            ...
            if (org && org.hierarchyLevel) {
              const level = { 'union': 1, 'conference': 1, 'church': 2 }[org.hierarchyLevel] || 2;
  ...
  async applyIndexes() {
    console.log('  Running index migration script...');
  ...
  async rollback() {
    console.log('  Rollback initiated (would restore from backup in production)');
  }
  ...
  async logMigration() {
    await AuditLog.logAction({
      ...
      result: {
        ...
        affectedCount: Object.values(this.stats).reduce((sum, stat) => sum + stat.updated, 0)
      },
      changes: this.stats,
      compliance: {
  ...
  printValidationResults(validation) {
    ...
    const categories = ['orphanedEntities', 'circularDependencies', 'invalidPaths', 'depthMismatches', 'missingParents'];
    
    categories.forEach(category => {
      if (validation[category] && validation[category].length > 0) {
        console.log(`  ${category}: ${validation[category].length} issues found`);
        if (this.verbose) {
          validation[category].slice(0, 5).forEach(issue => {
            console.log(`    - ${JSON.stringify(issue)}`);
          });
          if (validation[category].length > 5) {
            console.log(`    ... and ${validation[category].length - 5} more`);
          }
        }
      }
    });
  ...
  printSummary() {
    ...
    console.log('\n' + (this.dryRun ? 'DRY RUN COMPLETE' : 'MIGRATION COMPLETE'));
  }
}
