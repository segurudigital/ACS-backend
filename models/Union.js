...
unionSchema.methods.getFullHierarchy = async function() {
  ...
  const conferences = await Conference.find({ unionId: this._id, isActive: true })
...
unionSchema.methods.getStatistics = async function() {
  ...
  const stats = {
    conferences: await Conference.countDocuments({ unionId: this._id, isActive: true }),
    churches: 0,
    teams: 0,
    services: 0
  };
  ...
  const conferences = await Conference.find({ unionId: this._id, isActive: true }).select('_id');
  const conferenceIds = conferences.map(c => c._id);
  
  if (conferenceIds.length > 0) {
    // Count churches in these conferences
    const churches = await Church.find({ conferenceId: { $in: conferenceIds }, isActive: true }).select('_id');
...
unionSchema.pre('remove', async function(next) {
  const Conference = mongoose.model('Conference');
  const conferenceCount = await Conference.countDocuments({ unionId: this._id });
  
  if (conferenceCount > 0) {
    return next(new Error(`Cannot delete union: ${conferenceCount} conferences still exist`));
  }
...
unionSchema.pre('save', function(next) {
  ...
  const primaryLanguages = this.settings.languages?.filter(lang => lang.isPrimary) || [];
