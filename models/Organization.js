...
organizationSchema.pre('save', function (next) {
  ...
  if (this.type === 'union' && this.parentOrganization) {
    return next(
      new Error('Union organizations cannot have a parent organization')
    );
  }
  ...
  if (this.type === 'church' && !this.parentOrganization) {
    return next(
      new Error('Church organizations must have a parent conference')
    );
  }
...
organizationSchema.statics.getHierarchy = async function (organizationId) {
  const org =
    await this.findById(organizationId).populate('parentOrganization');
  ...
  while (current.parentOrganization) {
    current = await this.findById(current.parentOrganization._id).populate(
      'parentOrganization'
    );
