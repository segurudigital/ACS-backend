...
const auditLogSchema = new mongoose.Schema(
  {
    ...
    target: {
      ...
      id: {
        type: mongoose.Schema.Types.ObjectId,
        required: function () {
          return this.target.type !== 'system';
        },
      },
      name: String,
      hierarchyPath: String,
...
auditLogSchema.index({
  'hierarchyContext.hierarchyViolation': 1,
  timestamp: -1,
});
...
auditLogSchema.pre('save', function (next) {
  ...
  if (this.hierarchyContext.actorPath && this.hierarchyContext.targetPath) {
    this.hierarchyContext.crossHierarchy =
      !this.hierarchyContext.targetPath.startsWith(
        this.hierarchyContext.actorPath
      );
  }
  ...
  if (this.network.ipAddress) {
    this.network.ipVersion = this.network.ipAddress.includes(':')
      ? 'IPv6'
      : 'IPv4';
  }
...
auditLogSchema.statics.getActionsOn = async function (
  targetType,
  targetId,
  options = {}
) {
