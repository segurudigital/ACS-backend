...
class AuditLogger {
    ...
    return async (req, res, next) => {
      ...
      if (originalCheckPermission) {
        req.checkPermission = async function(permission, targetEntity) {
          const startTime = Date.now();
          const result = await originalCheckPermission.call(this, permission, targetEntity);
          
          // Log permission escalations and failures
          if (!result || (targetEntity && AuditLogger._isPermissionEscalation(req.user, targetEntity))) {
            const action = result ? 'permission.escalation' : 'permission.check_failed';
      ...
      if (entityType && ['POST', 'PUT', 'PATCH', 'DELETE'].includes(req.method)) {
        ...
        res.end = function(...args) {
          if (res.statusCode < 400) {
            const action = AuditLogger._getActionFromMethod(req.method, hierarchyEndpoints[entityType]);
              ...
              if (entity && entity._id) {
                req.auditLog(action, {
                  ...
                  changes: req.method === 'PUT' || req.method === 'PATCH' ? 
                    AuditLogger._extractChanges(req.body) : undefined
                });
              }
            } catch (e) {
  ...
  static _determineRetentionCategory(action) {
    ...
    if (action.includes('.delete') || action.includes('.role') || action.includes('hierarchy.')) {
      return 'compliance';
    }
  ...
  static _extractChanges(body) {
    ...
    const trackedFields = ['name', 'status', 'hierarchyLevel', 'parentOrganization', 'churchId', 'teamId'];
  ...
  static _isPermissionEscalation(user, targetEntity) {
    ...
    if (targetEntity.hierarchyLevel !== undefined && user.hierarchyLevel !== undefined) {
