...
class HierarchyMigrationService {
  ...
  async rebuildAllHierarchyPaths(options = {}) {
    ...
    try {
      ...
      // Then conferences
      const conferences = await this.Conference.find({})
        .populate('unionId')
        .session(session);
      ...
      // Finally churches
      const churches = await this.Church.find({})
      ...
      for (const church of churches) {
        ...
        const parent = await this.Conference.findById(
          church.conferenceId._id
        ).session(session);
      ...
      // Rebuilding team hierarchy paths
      ...
      // Rebuilding service hierarchy paths
      ...
      for (const service of services) {
        ...
        const team = await this.Team.findById(service.teamId._id).session(
          session
        );
      ...
      if (!dryRun) {
        ...
        // Hierarchy paths rebuilt successfully
      } else {
    ...
    try {
      ...
      switch (entityType.toLowerCase()) {
        case 'organization': {
          ...
          newParent =
            await this.Organization.findById(newParentId).session(session);
          ...
          // Validate the move
          const validation = HierarchyValidator.validateOrganizationHierarchy(
            entity,
            newParent
          );
  ...
  async validateHierarchyIntegrity() {
    ...
    for (const org of organizations) {
      ...
      const expectedDepth = { union: 0, conference: 1, church: 2 }[
        org.hierarchyLevel
      ];
